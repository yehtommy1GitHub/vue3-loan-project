...
| 3.6 | 2026/06/10 21:23:34 | 建立 API SDK 與發票平台 API 本地套件，提供 `.tgz` 安裝、型別 declaration、單元測試與跨專案使用手冊 | Codex |
| 3.7 | 2026/06/10 21:36:51 | 新增 Vue3 技術展示文件，說明元件溝通、路由導航、mock + Axios 串接與 Pinia Store 全域狀態管理 | Codex |
...
- 將通用 API SDK 與發票平台業務 API 打包為本地 `.tgz` 套件，供其他 Vue3 或 TypeScript 專案安裝使用。
- 透過 `vue3技術展示文件.md` 說明 props/emit、Vue Router、mock + Axios 與 Pinia Store 的實務用途；現行全域狀態仍採用 Vuex。
- 前端單元測試。
...
| 套件 | 套件名稱 | 規格 |
|---|---|---|
| 通用 API SDK | `@vue3-invoice/api-sdk` | 提供可注入 Axios Client、端點設定、動態路徑解析、統一 request、錯誤正規化與 `success` 判斷。 |
| 發票平台 API | `@vue3-invoice/platform-api` | 提供登入、註冊、使用者查詢、發票放款更新、異動紀錄查詢、匯率查詢與端點覆寫。 |

本地套件使用 Vite library mode 建置 JavaScript，並使用 TypeScript 產生 declaration。執行 `npm run pack:packages` 後，需在 `local-packages/` 產生版本化 `.tgz`。發票平台 API 已內建 API SDK 核心，可單獨安裝使用；通用 API SDK 則供其他業務模組自行建立 API Client。

本地套件交付規則：

- 公開函式、參數、回傳值與資料契約必須具備 TypeScript 型別。
- `.tgz` 與套件 `dist/` 為建置產物，不得上傳 GitHub。
- 套件交付前需通過單元測試、build、pack 與獨立安裝 smoke test。
- API URL、token 與環境資訊由使用端注入，不得寫死真實環境敏感設定。
...
| AT-025 | API 本地套件打包 | 執行 `npm run pack:packages` | 產生 API SDK 與發票平台 API 版本化 `.tgz`，內容包含 JavaScript、TypeScript declaration、README 與 package.json |
| AT-026 | 本地套件獨立安裝 | 在最小獨立專案安裝兩個 `.tgz` 並執行 smoke test | 可成功 import、建立 API 實例並透過注入 Client 呼叫 |
...
2026/06/10 21:26:16 本地套件驗證結果：

- `npm run test:packages`：2 個套件測試檔、9 個測試案例通過。
- `npm run pack:packages`：成功產生 `vue3-invoice-api-sdk-1.0.0.tgz` 與 `vue3-invoice-platform-api-1.0.0.tgz`。
- 獨立 smoke-test：從 `.tgz` 安裝後，可成功 import、建立 API 實例、呼叫並通過 TypeScript declaration 檢查。
- 完整 `npm test`：11 個測試檔、46 個測試案例通過。
- `npm run type-check` 與 `npm run build`：通過。
