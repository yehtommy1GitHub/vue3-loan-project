...
後端 API 的 method、URL 與動態路徑集中於 `src/config/backendApiConfig.ts`，`src/services/authApi.ts` 僅依 API 名稱取得設定並呼叫。若 `VITE_USE_BACKEND_API=true` 但未設定 `VITE_BACKEND_API_BASE_URL`，系統會回報設定錯誤，不可 fallback 回 mock API。
