...
// API 設定層：集中讀取 Vite 環境變數與端點設定，讓 API client 與業務 API 不直接依賴 .env 細節。
...
export const backendApiConfig = {
  ...
  endpoints: {
    ...
    fetchUser: {
      ...
      method: 'GET',
      url: ({ account }: EndpointParams): string => `/users/${encodeURIComponent(account ?? '')}`,
      healthCheck: {
        enabled: true,
        params: {
          account: 'DEMO0001'
    ...
    updateLoans: {
      ...
      method: 'PUT',
      url: ({ account }: EndpointParams): string => `/users/${encodeURIComponent(account ?? '')}/loans`,
      healthCheck: {
        enabled: false,
        params: {
          account: 'DEMO0001'
    ...
    fetchLoanChangeLogs: {
      ...
      method: 'GET',
      url: ({ account }: EndpointParams): string => `/users/${encodeURIComponent(account ?? '')}/loan-change-logs`,
      healthCheck: {
        enabled: true,
        params: {
          account: 'DEMO0001'
...
export type BackendApiEndpointName = keyof typeof backendApiConfig.endpoints;

// 依 API 名稱與參數解析實際 URL 與 HTTP method，讓業務 API 層不需要知道路徑組裝細節。
